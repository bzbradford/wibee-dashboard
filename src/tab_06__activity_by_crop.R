    ...
    function(input, output, session) {
      ...
      output$ui <- renderUI({
        ...
        tagList(
          ...
          div(
            class = "plot-caption",
            "This interactive chart compares total pollinator visitation rates across all of the different crops and non-crop plants surveyed with the app. The number of surveys represented by each plant species or group is shown in parentheses in the labels."
          )
        )
      })
      ...
      output$plot <- renderPlotly({
        ...
        margin <- min(
          max(40, 10 + 4 * max(nchar(data_long()$plant_label))),
          200
        )
        ...
        plot_data |>
          ...
          layout(
            barmode = "stack",
            title = list(
              text = "<b>Pollinator visitation rates by plant type</b>",
              font = list(size = 15)
            ),
            xaxis = list(title = "", fixedrange = TRUE, tickangle = 45),
            yaxis = list(
