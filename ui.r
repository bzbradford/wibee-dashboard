...
ui <- fixedPage(
  ...
  tags$head(
    ...
    tags$meta(
      name = "description",
      content = "An online data dashboard for viewing pollinator activity surveys recorded by the WiBee app"
    ),
    tags$meta(
      name = "keywords",
      content = "insect, pollinator, bee, dashboard, wisconsin, wibee, wisconsin"
    ),
  ...
  div(
    ...
    p(
      ...
      "WiBee is a project of the",
      lnk(
        "UW-Madison Department of Entomology",
        "https://entomology.wisc.edu/"
      ),
      "and the",
      lnk("Gratton Lab", "https://gratton.entomology.wisc.edu/"),
      br(),
      ...
      " - ",
      "WiBee app developed by",
      lnk("Dan Imhoff", "https://caracal.tech/"),
    ),
    br(),
    p(
      em(status),
      ...
      em(paste(
        "Data last updated:",
        format(refresh_time, "%Y-%m-%d %H:%M:%S %Z")
      ))
    )
  )
)
