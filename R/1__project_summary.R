...
introUI <- function() {
  main_panel <- mainPanel(
    ...
    p(
      "WiBee (pronounced We-bee) is a new smartphone app developed by the",
      a(
        "Gratton Lab",
        href = "https://gratton.entomology.wisc.edu/",
        target = "_blank"
      ),
    ...
    p(
      ...
      a(
        "Send feedback.",
        href = "https://forms.gle/6qy9qJLwCxSTTPNT8",
        target = "_blank"
      ),
      "Want to stay in the loop?",
      a(
        "Sign up for our newsletter.",
        href = "http://eepurl.com/gMqRdr",
        target = "_blank"
      ),
  ...
  sidebar <- sidebarPanel(
    ...
    p(
      "Contributors: ",
      format(length(unique(surveys$user_id)), big.mark = ",")
    ),
    p("Total completed surveys: ", format(nrow(surveys), big.mark = ",")),
    p("Most recent survey: ", format(max(surveys$date), "%b %d, %Y")),
    p(
      "Total insect observations: ",
      format(sum(surveys$total_visits), big.mark = ",")
    ),
    tags$ul(
  ...
  how_to_use <- bsCollapse(
    bsCollapsePanel(
      ...
      p(
        ...
        strong("selecting surveys"),
        " and ",
        strong("viewing survey data.")
      ),
      p(
      ...
      tags$ul(
        ...
        tags$li(
          strong("Select or group bee types."),
          " This is optional, but you can group wild bees together (bumble bees, dark bees, and green bees), or select specific bee groups to show on the plots."
        ),
      ),
      p(
